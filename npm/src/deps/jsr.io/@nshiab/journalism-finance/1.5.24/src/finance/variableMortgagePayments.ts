 ...
 * @category Finance
 */
export default function variableMortgagePayments(
  ...
  if (rates.length < numberOfPaymentsinTerm) {
    throw new Error(
      `Not enough rates provided. Expected at least ${numberOfPaymentsinTerm} rates for a ${term}-year term, but got ${rates.length}.`,
    );
  }
  ...
  if (
    typeof annualCompounding !== "number" ||
    annualCompounding <= 0
  ) {
  ...
  const calculateMonthlyRate = (rate: number): number => {
    const nominalRate = rate / 100;
    const annualEffectiveRate = Math.pow(
      1 + nominalRate / annualCompounding,
      annualCompounding,
    );
