 ...
 * @category Google
 */
async function toBucket(file, destination, options = {}) {
    ...
    if (options.skip) {
        const localFileExists = (0, node_fs_1.existsSync)(file);
        ...
        if (!localFileExists && !exists) {
            ...
            throw new Error(`Local file '${file}' does not exist. Cannot upload to bucket.`);
        }
        if (exists) {
        ...
        throw new Error(`File '${destination}' already exists in bucket '${bucketName}'. Set overwrite: true to replace it or skip: true to skip upload.`);
    }
    await b.upload(file, {
        destination,
