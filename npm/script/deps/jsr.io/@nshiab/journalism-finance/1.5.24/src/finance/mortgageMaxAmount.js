...
exports.default = mortgageMaxAmount;
 ...
 * @category Finance
 */
function mortgageMaxAmount(annualIncome, downPayment, rate, options = {}) {
    ...
    findMaxAmount(downPayment, 100_000, monthlyIncome, downPayment, monthlyRate, amortizationPeriodinMonths, monthlyHeating, monthlyCondoFees, monthlyDebtPayment, options, results);
    // Then with the purchase price previously found but we increment by $10,000
    findMaxAmount(results.purchasePrice, 10_000, monthlyIncome, downPayment, monthlyRate, amortizationPeriodinMonths, monthlyHeating, monthlyCondoFees, monthlyDebtPayment, options, results);
    // Again but $1,000 increment
    findMaxAmount(results.purchasePrice, 1_000, monthlyIncome, downPayment, monthlyRate, amortizationPeriodinMonths, monthlyHeating, monthlyCondoFees, monthlyDebtPayment, options, results);
...
function findMaxAmount(startPrice, increment, monthlyIncome, downPayment, monthlyRate, amortizationPeriodinMonths, monthlyHeating, monthlyCondoFees, monthlyDebtPayment, options, results) {
    // We start with zeros
    let grossDebtServiceRatio = 0;
    let totalDebtServiceRatio = 0;
    // We test amount up to $10,000,000
    let downPaymentTooLow = false;
    for (let purchasePrice = startPrice; purchasePrice <= 10_000_000; purchasePrice += increment) {
        // We check that the down payment is enough
        const downPaymentMin = (0, getMinimumDownPayment_js_1.default)(purchasePrice);
        ...
        const insurancePremium = (0, mortgageInsurancePremium_js_1.default)(purchasePrice, downPayment);
        // Then the actual mortgage amount
        const mortgageAmount = Math.max(purchasePrice - downPayment, 0) +
            insurancePremium;
        // And monthly mortgage payment
        const monthlyMortgagePayment = Math.round((monthlyRate * mortgageAmount) /
        ...
        if (grossDebtServiceRatio >= 0.32 || totalDebtServiceRatio >= 0.4) {
            ...
            results.grossDebtServiceRatio = parseFloat(grossDebtServiceRatio.toFixed(2));
            results.totalDebtServiceRatio = parseFloat(totalDebtServiceRatio.toFixed(2));
        }
    }
}
