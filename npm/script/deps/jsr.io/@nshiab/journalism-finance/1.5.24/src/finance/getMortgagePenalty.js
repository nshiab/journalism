...
exports.default = getMortgagePenalty;
 ...
 * ```
 */
function getMortgagePenalty(parameters) {
    const { remainingMonthsToTerm, mortgageBalance, postedInterestRate, rateAdjustmentFixed, rateAdjustmentVariable, currentPostedRates, mortgageType, } = parameters;
    // If term is done, no penalty
    if (remainingMonthsToTerm === 0) {
        return 0;
    }
    if (rateAdjustmentFixed !== 0 && rateAdjustmentVariable !== 0) {
        throw new Error("Both fixed and variable rate adjustments cannot be non-zero at the same time.");
    }
    const effectiveRate = Math.max(0, postedInterestRate + rateAdjustmentFixed +
        rateAdjustmentVariable);
    if (mortgageType === "variable") {
        // Three months interest penalty
        const threeMonthsPenalty = (0, index_js_1.round)((mortgageBalance * effectiveRate * 3) / 12, R2);
        ...
        const comparisonRate = currentPostedRates[termForRate === 0 ? 1 : termForRate];
        if (comparisonRate === undefined) {
            throw new Error(`No current posted rate provided for a ${termForRate} year term.`);
        }
        // Three months interest penalty
        const threeMonthsPenalty = (0, index_js_1.round)((mortgageBalance * effectiveRate * 3) / 12, R2);
        // IRD (Interest Rate Differential) penalty
        const irdRate = Math.max(0, effectiveRate - (comparisonRate + rateAdjustmentFixed));
        const fixedMortgagePenalty = (0, index_js_1.round)(mortgageBalance * irdRate * remainingYearsToTerm, R2);
