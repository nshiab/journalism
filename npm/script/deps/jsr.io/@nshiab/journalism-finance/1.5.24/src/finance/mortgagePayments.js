...
exports.default = mortgagePayments;
 ...
 * @category Finance
 */
function mortgagePayments(mortgageAmount, rate, paymentFrequency, term, amortizationPeriod, options = {}) {
    ...
    const annualEffectiveRate = Math.pow(1 + nominalRate / options.annualCompounding, options.annualCompounding);
    // A function to compute the rate for a given time interval
    const computePeriodicRate = (interval) => {
