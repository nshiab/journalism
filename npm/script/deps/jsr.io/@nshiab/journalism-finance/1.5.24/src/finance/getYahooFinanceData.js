...
exports.default = getYahooFinanceData;
 ...
 * @category Finance
 */
async function getYahooFinanceData(symbol, startDate, endDate, variable, interval, useBrowser = false) {
    const period1 = Math.round(startDate.getTime() / 1000);
    const period2 = Math.round(endDate.getTime() / 1000);
    const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?events=capitalGain%7Cdiv%7Csplit&formatted=true&includeAdjustedClose=true&interval=${interval}&period1=${period1}&period2=${period2}&symbol=${symbol}&userYfid=true&lang=en-CA&region=CA`;
    const headers = {};
    if (useBrowser) {
        headers["User-Agent"] =
    ...
    if (!response.ok) {
        ...
        throw new Error(`Failed to fetch data: ${response.status} ${response.statusText}${text ? `. ${text}` : ""}${!useBrowser ? ". Try setting the 'useBrowser' option to true." : ""}`);
    }
    const data = await response.json();
    if (data.chart.result.length === 0) {
        throw new Error("No data found.");
    }
    const timestamp = data.chart.result[0].timestamp.map((d) => d * 1000);
    let value;
    if (variable === "adjclose") {
        if (!data.chart.result[0].indicators.adjclose) {
            throw new Error("adjclose data not available. Please use 'open', 'high', 'low', 'close' or 'volume'.");
        }
        value = data.chart.result[0].indicators.adjclose[0].adjclose;
    }
    else if (variable === "open" || variable === "high" || variable === "low" ||
        variable === "close" || variable === "volume") {
        value = data.chart.result[0].indicators.quote[0][variable];
    }
    else {
        throw new Error("Unknown variable. Please use 'open', 'high', 'low', 'close', 'adjclose' or 'volume'.");
    }
    const rows = (0, index_js_1.arraysToData)({ timestamp, value });
    return rows;
}
