...
exports.default = getRentVsBuyCholeskyMatrix;
const index_js_1 = require("../../../../../../journalism-statistics/1.5.2/src/index.js");
/**
 ...
 * ```
 */
function getRentVsBuyCholeskyMatrix(data, options = {}) {
    const numVars = 16;
    if (!data) {
        const identityMatrix = Array.from({ length: numVars }, (_, i) => Array.from({ length: numVars }, (_, j) => (i === j ? 1 : 0)));
        return (0, index_js_1.getCholeskyMatrix)(identityMatrix, options.jitter);
    }
    const arrays = [
    ...
    for (let i = 1; i < numVars; i++) {
        if (arrays[i].length !== length) {
            throw new Error("All historical data arrays must have the exact same length.");
        }
    }
    // Transpose to observations: rows = time, cols = variables
    const observations = new Array(length);
    ...
    const corMatrix = (0, index_js_1.getCorrelationMatrix)(observations);
    return (0, index_js_1.getCholeskyMatrix)(corMatrix, options.jitter);
}
