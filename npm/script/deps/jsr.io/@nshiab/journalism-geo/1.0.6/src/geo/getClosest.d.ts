 ...
 * @category Geo
 */
export default function getClosest<T>(lon: number, lat: number, geoItems: T[], getItemLon: (item: T) => number, getItemLat: (item: T) => number, options?: {
    addDistance?: false;
    decimals?: number;
}): T;
/**
 ...
 */
export default function getClosest<T extends {
    properties: unknown;
}>(lon: number, lat: number, geoItems: T[], getItemLon: (item: T) => number, getItemLat: (item: T) => number, options: {
    addDistance: true;
    decimals?: number;
}): T & {
