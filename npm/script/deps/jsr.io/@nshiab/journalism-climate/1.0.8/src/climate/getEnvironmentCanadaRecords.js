...
exports.default = getEnvironmentCanadaRecords;
const index_js_1 = require("../../../../journalism-geo/1.0.6/src/index.js");
 ...
 * @category Weather
 */
async function getEnvironmentCanadaRecords(locations, variable, dateRange, options = {}) {
    const delay = options.delay ?? 100; // Default delay in milliseconds
    const startDate = new Date(Date.UTC(parseInt(dateRange[0].split("-")[0]), parseInt(dateRange[0].split("-")[1]) - 1, parseInt(dateRange[0].split("-")[2])));
    const endDate = new Date(Date.UTC(parseInt(dateRange[1].split("-")[0]), parseInt(dateRange[1].split("-")[1]) - 1, parseInt(dateRange[1].split("-")[2])));
    const allStationsResponse = await fetch("https://api.weather.gc.ca/collections/ltce-stations/items?f=json&limit=9999999");
    const allStations = await allStationsResponse.json();
    const allStationsWithVariable = allStations.features.filter((d) => d.properties.ELEMENT_NAME_E === variable);
    const stationsIdsAndCoordinates = [];
    const returnedData = [];
    for (const station of allStationsWithVariable) {
        if (!stationsIdsAndCoordinates.some((s) => s.id === station.properties.VIRTUAL_CLIMATE_ID)) {
    ...
    for (const location of locations) {
        ...
        if (typeof lat !== "number" || typeof lon !== "number") {
            throw new Error("Each location must have numeric lat and lon properties.");
        }
        const closestStation = (0, index_js_1.getClosest)(lon, lat, stationsIdsAndCoordinates, (d) => d.lon, (d) => d.lat, {
            addDistance: true,
            decimals: 0,
        });
        if (options.verbose) {
        ...
        const numberOfDays = Math.ceil((endDate.getTime() - startDate.getTime()) /
            (1000 * 60 * 60 * 24));
        for (let i = 0; i <= numberOfDays; i++) {
            ...
            const url = `https://api.weather.gc.ca/collections/${ltceCollection}/items/${closestStation.id}-${monthDay}?f=json`;
            if (options.verbose) {
            ...
            let recordValue;
            let recordYear;
            let previousRecordValue;
            let previousRecordYear;
            let recordStationRecordBegin;
            let recordStationRecordEnd = null;
            if (variable === "DAILY MAXIMUM TEMPERATURE") {
