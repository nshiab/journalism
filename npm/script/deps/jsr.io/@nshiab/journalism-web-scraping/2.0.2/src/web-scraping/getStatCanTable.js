...
exports.default = getStatCanTable;
 ...
 */
async function getStatCanTable(pid, options = {}) {
    const lang = options.lang ?? "en";
    if (pid.length > 8) {
        options.debug &&
            console.log(`pid ${pid} is too long (max 8 characters). Using ${pid.slice(0, 8)} instead.`);
        pid = pid.slice(0, 8);
    }
    const getCSVResponse = await fetch(`https://www150.statcan.gc.ca/t1/wds/rest/getFullTableDownloadCSV/${pid}/${lang}`);
    ...
    const zipBuffer = node_buffer_1.Buffer.from(await response.arrayBuffer());
