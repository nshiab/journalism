 ...
 * ```
 */
async function getHtmlTable(urlOrHtml, options = {}) {
    let html = "";
    if (urlOrHtml.startsWith("http://") || urlOrHtml.startsWith("https://")) {
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch ${urlOrHtml}: ${response.status} ${response.statusText}`);
        }
    ...
    if (typeof options.selector === "string") {
        table = $(options.selector).filter((i) => i === (typeof options.index === "number" ? options.index : 0));
    }
    else {
        table = $("table").filter((i) => i === (typeof options.index === "number" ? options.index : 0));
    }
    let csv = "";
    table.find("tr").each((_i, tr) => {
        ...
        csv += `${(0, d3_dsv_1.csvFormatRow)(row)}\n`;
    });
    const data = (0, d3_dsv_1.csvParse)(csv);
    delete data["columns"];
    return data;
}
