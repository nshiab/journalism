...
exports.default = getGbmParameters;
/**
 ...
 * ```
 */
function getGbmParameters(values, periodsPerYear) {
    if (values.length < 3)
        throw new Error("Need at least three data points.");
    ...
    const variance = logReturns.reduce((acc, val) => acc + (val - meanReturn) ** 2, 0) /
        (logReturns.length - 1);
