...
exports.default = addZScore;
// Implementation
function addZScore(data, variable, options = {}) {
    // Average
    let sum = 0;
    ...
    const sqdDistFromMean = data
        .map((d) => Math.pow(d[variable] - mean, 2)) // we checked the type above
        .reduce((acc, curr) => (acc += curr), 0);
    const stdDev = Math.sqrt(sqdDistFromMean / data.length);
    // Z-Score
    const newKey = (options.newKey ?? "zScore");
    for (let i = 0; i < data.length; i++) {
        data[i][newKey] =
            (data[i][variable] - mean) / stdDev; // we checked the type above
    }
    return data;
}
