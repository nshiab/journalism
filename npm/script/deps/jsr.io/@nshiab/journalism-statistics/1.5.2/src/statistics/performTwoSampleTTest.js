 ...
 * @category Statistics
 */
function performTwoSampleTTest(group1Data, group2Data, variableKey, options = {}) {
    // --- 1. Helper function to safely extract and validate numeric data ---
    const extractNumericValues = (data, key, groupName) => {
        const values = [];
        data.forEach((item, index) => {
            const value = item[key];
            if (typeof value !== "number" || !isFinite(value)) {
                throw new Error(`Invalid data in ${groupName} at index ${index}. Expected a finite number for key "${String(key)}", but received: ${JSON.stringify(value)}.`);
            }
            values.push(value);
        });
        return values;
    ...
    const calculateSampleVariance = (data, mean) => {
    ...
    if (group1Variance === 0 && group2Variance === 0) {
        if (group1Mean === group2Mean) {
            ...
            throw new Error("Cannot perform t-test: both groups have zero variance (all values are identical within each group) but different means. This indicates a perfect separation between groups.");
        }
    }
    ...
    const standardError = Math.sqrt(group1Variance / group1SampleSize + group2Variance / group2SampleSize);
    const tStatistic = meanDifference / standardError;
    // Welch-Satterthwaite equation for degrees of freedom
    const numerator = Math.pow(group1Variance / group1SampleSize + group2Variance / group2SampleSize, 2);
    const denominator = Math.pow(group1Variance / group1SampleSize, 2) / (group1SampleSize - 1) +
    ...
    if (tail === "two-tailed") {
        ...
        throw new Error(`Invalid tail option: ${tail}. Use "two-tailed", "left-tailed", or "right-tailed".`);
    }
    return {
        group1SampleSize,
        group2SampleSize,
