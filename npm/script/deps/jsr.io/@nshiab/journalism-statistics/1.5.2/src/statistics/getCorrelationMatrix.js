...
exports.default = getCorrelationMatrix;
const invertMatrix_js_1 = __importDefault(require("./invertMatrix.js"));
 ...
 * @category Statistics
 */
function getCorrelationMatrix(data, options = {}) {
    const covMatrix = (0, getCovarianceMatrix_js_1.default)(data);
    const m = covMatrix.length;
    // Initialize correlation matrix
    const corrMatrix = Array(m)
        .fill(null)
        .map(() => Array(m).fill(0));
        ...
        for (let j = 0; j < m; j++) {
            if (i === j) {
                ...
                const correlation = stdDevs[i] === 0 || stdDevs[j] === 0
                    ? 0
                    : covMatrix[i][j] / (stdDevs[i] * stdDevs[j]);
                // Clamp to [-1, 1] to handle floating point errors
                const clampedCorrelation = Math.max(-1, Math.min(1, correlation));
                corrMatrix[i][j] = clampedCorrelation;
                corrMatrix[j][i] = clampedCorrelation; // Matrix is symmetric
            }
        }
    }
    return options.invert ? (0, invertMatrix_js_1.default)(corrMatrix) : corrMatrix;
}
