 ...
 * @category Statistics
 */
function performZTest(populationData, sampleData, variableKey, options = {}) {
    // --- 1. Helper function to safely extract and validate numeric data ---
    const extractNumericValues = (data, sourceName) => {
        return data.map((item, index) => {
            const value = item[variableKey];
            if (typeof value !== "number" || !isFinite(value)) {
                throw new Error(`Invalid data in ${sourceName} array at index ${index}. Expected a finite number for key "${String(variableKey)}", but received: ${JSON.stringify(value)}.`);
            }
            return value;
        });
    };
    ...
    if (populationSize < 2 || sampleSize < 2) {
        throw new Error("Population and sample must contain at least 2 data points.");
    }
    ...
    const populationVariance = calculateVariance(populationValues, populationMean);
    ...
    const cdf = (val) => 0.5 * (1 + erf(val / Math.sqrt(2)));
    let pValue;
    if (tail === "two-tailed") {
