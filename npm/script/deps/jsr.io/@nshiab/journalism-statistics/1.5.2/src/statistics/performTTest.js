 ...
 */
const extractNumericValues = (data, variableKey) => {
    return data.map((item, index) => {
        const value = item[variableKey];
        if (typeof value !== "number" || !isFinite(value)) {
            throw new Error(`Invalid data in sample array at index ${index}. Expected a finite number for key "${String(variableKey)}", but received: ${JSON.stringify(value)}.`);
        }
        return value;
    });
};
 ...
 */
const calculateMeanAndVariance = (data) => {
    if (data.length === 0) {
 ...
 * @category Statistics
 */
function performTTest(sampleData, variableKey, hypothesizedMean, options = {}) {
    // --- 1. Validate hypothesized mean ---
    if (typeof hypothesizedMean !== "number" || !isFinite(hypothesizedMean)) {
        throw new Error(`Invalid hypothesized mean. Expected a finite number, but received: ${JSON.stringify(hypothesizedMean)}.`);
    }
    ...
    if (sampleSize < 2) {
        throw new Error("Sample must contain at least 2 data points.");
    }
    // --- 3. Calculate sample statistics using single-pass algorithm ---
    const { mean: sampleMean, variance: sampleVariance } = calculateMeanAndVariance(sampleValues);
