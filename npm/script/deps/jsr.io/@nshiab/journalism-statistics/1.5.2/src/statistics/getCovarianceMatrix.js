...
exports.default = getCovarianceMatrix;
 ...
 * @category Statistics
 */
function getCovarianceMatrix(data, options = {}) {
    const n = data.length;
    const m = data[0].length;
    // Initialize sums
    const sums = Array(m).fill(0);
    const squaredSums = Array(m).fill(0);
    const productSums = Array(m)
        .fill(null)
        .map(() => Array(m).fill(0));
    // Calculate sums
    for (let i = 0; i < n; i++) {
    ...
    return options.invert ? (0, invertMatrix_js_1.default)(covMatrix) : covMatrix;
}
