...
exports.default = generateGbmPath;
/**
 ...
 * ```
 */
function generateGbmPath(startValue, mu, sigma, years, periodsPerYear) {
    // Input guard rails
    if (startValue <= 0 || sigma < 0 || periodsPerYear <= 0) {
        throw new Error("Invalid inputs: startValue and periodsPerYear must be > 0, sigma must be >= 0.");
    }
    const dt = 1 / periodsPerYear;
    const totalSteps = Math.round(years * periodsPerYear); // Ensures integer steps
    const path = new Array(totalSteps + 1); // Pre-allocating array size is faster than .push()
    path[0] = startValue;
