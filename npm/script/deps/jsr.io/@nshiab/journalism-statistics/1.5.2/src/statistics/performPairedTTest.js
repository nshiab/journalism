...
exports.default = performPairedTTest;
 ...
 * @category Statistics
 */
function performPairedTTest(pairedData, firstVariableKey, secondVariableKey, options = {}) {
    // --- 1. Helper function to safely extract and validate numeric data ---
    const extractPairedNumericValues = (data, key1, key2) => {
        const first = [];
        const second = [];
        data.forEach((item, index) => {
            ...
            if (typeof value1 !== "number" || !isFinite(value1)) {
                throw new Error(`Invalid data at index ${index}. Expected a finite number for key "${String(key1)}", but received: ${JSON.stringify(value1)}.`);
            }
            if (typeof value2 !== "number" || !isFinite(value2)) {
                throw new Error(`Invalid data at index ${index}. Expected a finite number for key "${String(key2)}", but received: ${JSON.stringify(value2)}.`);
            }
            first.push(value1);
            second.push(value2);
        });
    ...
    const { first, second } = extractPairedNumericValues(pairedData, firstVariableKey, secondVariableKey);
    const sampleSize = first.length;
    if (sampleSize < 2) {
        throw new Error("Paired data must contain at least 2 pairs.");
    }
    ...
    const differenceVariance = (differenceSumSquared - (differenceSum * differenceSum) / sampleSize) /
        (sampleSize - 1);
