...
exports.default = performChiSquaredGoodnessOfFitTest;
const jstat_1 = __importDefault(require("jstat"));
// Extract just the chi-square distribution functions we need
const { chisquare } = jstat_1.default;
/**
 ...
 * @category Statistics
 */
function performChiSquaredGoodnessOfFitTest(data, categoryKey, observedKey, expectedKey) {
    // --- 1. Input validation ---
    if (!Array.isArray(data) || data.length === 0) {
        throw new Error("Data must be a non-empty array.");
    }
    // --- 2. Helper function to extract and validate data ---
    const extractValue = (item, key, index, expectedType) => {
        const value = item[key];
        if (expectedType === "number") {
            if (typeof value !== "number" || !isFinite(value) || value < 0) {
                throw new Error(`Invalid data at index ${index}. Expected a non-negative finite number for key "${key}", but received: ${JSON.stringify(value)}.`);
            }
            return value;
        }
        else if (expectedType === "string") {
            if (typeof value !== "string" && typeof value !== "number") {
                throw new Error(`Invalid data at index ${index}. Expected a string or number for key "${key}", but received: ${JSON.stringify(value)}.`);
            }
            return value;
        }
        throw new Error(`Invalid expectedType: ${expectedType}. Must be "number" or "string".`);
    };
    // --- 3. Process data ---
    const observedFreqs = {};
    const expectedFreqs = {};
    data.forEach((item, index) => {
        const category = String(extractValue(item, String(categoryKey), index, "string"));
        const observed = extractValue(item, String(observedKey), index, "number");
        const expected = extractValue(item, String(expectedKey), index, "number");
    ...
    categories.forEach((category) => {
        if (expectedFreqs[category] <= 0) {
            throw new Error(`Expected frequency must be greater than 0 for category "${category}".`);
        }
    });
    // Check if total observed equals total expected (allowing small rounding differences)
    const totalObserved = Object.values(observedFreqs).reduce((sum, freq) => sum + freq, 0);
    const totalExpected = Object.values(expectedFreqs).reduce((sum, freq) => sum + freq, 0);
    ...
    if (relativeDifference > 0.001) {
        throw new Error(`Total observed frequencies (${totalObserved}) must approximately equal total expected frequencies (${totalExpected}). ` +
    ...
    if (belowOne.length > 0) {
        warnings.push(`Warning: ${belowOne.length} expected frequencies are less than 1. Chi-squared test assumptions may be violated.`);
    }
    ...
    if (percentageAtLeastFive < 80) {
        warnings.push(`Warning: Only ${percentageAtLeastFive.toFixed(1)}% of expected frequencies are ≥ 5 (recommended: ≥ 80%). Results may be unreliable.`);
    }
    // Special check for df = 1 cases
    if (totalCategories === 2) {
        const belowFive = expectedValues.filter((freq) => freq < 5);
        if (belowFive.length > 0) {
            warnings.push(`Warning: For tests with 1 degree of freedom, all expected frequencies should be ≥ 5. Found ${belowFive.length} frequencies below 5.`);
        }
    }
    ...
    if (verySmall.length > 0) {
        warnings.push(`Warning: ${verySmall.length} expected frequencies are very small (< 0.5). Consider combining categories or collecting more data.`);
    }
 ...
 */
function calculateChiSquaredPValue(chiSquared, degreesOfFreedom) {
    if (degreesOfFreedom <= 0) {
