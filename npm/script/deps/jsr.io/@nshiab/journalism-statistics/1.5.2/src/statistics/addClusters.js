...
exports.default = addClusters;
function addClusters(data, minDistance, minNeighbours, distance, options = {}) {
    if (options.reset) {
        data.forEach((d) => {
            delete d.clusterId;
            delete d.clusterType;
        });
    }
    let clusterId = 0;
    ...
    for (const point of data) {
        if (point.clusterId === null) {
            const neighbours = getNeighbours(point);
            const clusterNeighbours = neighbours.find((n) => n.clusterType === "core");
            if (clusterNeighbours) {
                point.clusterId =
                    clusterNeighbours.clusterId;
    ...
    function expandCluster(neighbours, clusterLabel) {
        for (let i = 0; i < neighbours.length; i++) {
            ...
            if (neighbour.clusterId === undefined) {
                ...
                if (newNeighbours.length >= minNeighbours) {
                    ...
                    neighbours.push(...newNeighbours.filter((n) => !neighbours.includes(n)));
                }
                else {
