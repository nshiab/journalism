...
exports.default = getCholeskyMatrix;
/**
 ...
 */
function getCholeskyMatrix(matrix, jitter = 0) {
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j <= i; j++) {
            ...
            if (j === i) {
                ...
                if (val <= 0) {
                    throw new Error(`Matrix is not positive-definite. Try increasing the jitter (current: ${jitter}).`);
                }
                L[j][j] = Math.sqrt(val);
            }
            else {
