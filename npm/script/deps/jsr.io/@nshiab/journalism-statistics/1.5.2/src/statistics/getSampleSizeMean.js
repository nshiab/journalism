...
exports.default = getSampleSizeMean;
/**
 ...
 * @category Statistics
 */
function getSampleSizeMean(data, key, confidenceLevel, marginOfError, options) {
    let zScore;
    ...
    if (data.length < 2) {
        throw new Error("At least 2 data points are required to calculate sample standard deviation.");
    }
    const values = data.map((d) => {
        const value = d[key];
        if (typeof value !== "number") {
            throw new Error(`Invalid value for key "${String(key)}": ${value}. Values must be numbers.`);
        }
        return value;
    });
