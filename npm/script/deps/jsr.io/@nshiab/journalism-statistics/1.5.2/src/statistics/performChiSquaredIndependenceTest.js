...
exports.default = performChiSquaredIndependenceTest;
 ...
 * @category Statistics
 */
function performChiSquaredIndependenceTest(data, firstVariableKey, secondVariableKey, countKey) {
    // --- 1. Input validation ---
    ...
    data.forEach((item, index) => {
        const rowValue = String(extractValue(item, String(firstVariableKey), index, "string"));
        const colValue = String(extractValue(item, String(secondVariableKey), index, "string"));
        const count = extractValue(item, String(countKey), index, "number");
    ...
    // --- 4. Calculate totals ---
    const rowTotals = rows.map((row) => columns.reduce((sum, col) => sum + (contingencyData[row]?.[col] || 0), 0));
    const colTotals = columns.map((col) => rows.reduce((sum, row) => sum + (contingencyData[row]?.[col] || 0), 0));
    ...
    if (degreesOfFreedom === 1) {
        ...
        if (belowFive.length > 0) {
            warnings.push(`Warning: For 2x2 contingency tables, all expected frequencies should be ≥ 5. Found ${belowFive.length} frequencies below 5.`);
        }
    }
