...
exports.default = logLineChart;
const addLines_js_1 = __importDefault(require("./helpers/addLines.js"));
const prepChart_js_1 = __importDefault(require("./helpers/prepChart.js"));
/**
 ...
 * @category Dataviz
 */
function logLineChart(data, x, y, options = {}) {
    if (options.title) {
        console.log(`\n${options.title}`);
    }
    else {
        console.log(`\nLine chart of "${String(y)}" over "${String(x)}"${options.smallMultiples
            ? `, for each "${String(options.smallMultiples)}"`
            : ""}:`);
    }
    (0, prepChart_js_1.default)("line", data, String(x), String(y), addLines_js_1.default, {
        ...options,
