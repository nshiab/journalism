...
exports.default = validateDataTypes;
function validateDataTypes(data, x, y) {
    if (data.length === 0) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (!isValidX) {
            throw new Error(`Row ${i}: x-axis value "${x}" must be a number or Date. Got: ${typeof xValue} (${xValue})`);
        }
        // Validate y values
        const yValue = row[y];
        ...
        if (!isValidY) {
            throw new Error(`Row ${i}: y-axis value "${y}" must be a number. Got: ${typeof yValue} (${yValue})`);
        }
    }
}
