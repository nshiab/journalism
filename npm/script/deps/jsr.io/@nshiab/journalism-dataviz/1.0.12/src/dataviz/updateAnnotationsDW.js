...
exports.default = updateAnnotationsDW;
 ...
 * @category Dataviz
 */
async function updateAnnotationsDW(chartId, annotations, options = {}) {
    ...
    const annotationsWithDefaults = annotations.map((annotation) => {
        ...
        if (!annotation.x || !annotation.y || !annotation.text) {
            throw new Error("Missing x, y, or text for at least one annotation.");
        }
    ...
    if (response.status !== 200) {
        throw new Error(`updateAnnotationsDW ${chartId}: Upstream HTTP ${response.status} - ${response.statusText}`);
    }
}
