...
exports.default = logDotChart;
const addDots_js_1 = __importDefault(require("./helpers/addDots.js"));
 ...
 * @category Dataviz
 */
function logDotChart(data, x, y, options = {}) {
    if (options.title) {
        console.log(`\n${options.title}`);
    }
    else {
        console.log(`\nDot chart of "${String(y)}" over "${String(x)}"${options.smallMultiples
            ? `, for each "${String(options.smallMultiples)}"`
            : ""}:`);
    }
    (0, prepChart_js_1.default)("dot", data, String(x), String(y), addDots_js_1.default, {
        ...options,
