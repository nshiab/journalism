 ...
 */
function getAxisX(type, x, options) {
    ...
    if (xLabelFirst.length + 3 + xLabelLast.length > options.width) {
        throw new Error(`The x labels "${xLabelFirst}" and "${xLabelLast}" are overlapping. Format them using formatX or increase the width.`);
    }
    const xAxis = [];
