...
exports.default = saveChart;
const dntShim = __importStar(require("../../../../../../../_dnt.shims.js"));
const Plot = __importStar(require("@observablehq/plot"));
const d3 = __importStar(require("d3-array"));
...
chart, path, options = {}) {
    ...
    const { document, window, Node, Element, HTMLElement, SVGElement, CustomEvent, } = (0, linkedom_1.parseHTML)("<!DOCTYPE html><html><body></body></html>");
    const plotMarks = [
        "Area",
        "Arrow",
        "BarX",
    ...
    for (const key of keysToSet) {
        ...
        // deno-lint-ignore no-explicit-any
        oldGlobals[key] = dntShim.dntGlobalThis[key];
    }
    try {
        ...
        globalThis.d3 = d3;
        // @ts-ignore: setting globals
        globalThis.journalismFormat = { formatDate: index_js_1.formatDate, formatNumber: index_js_1.formatNumber, round: index_js_1.round };
        // @ts-ignore: setting globals
        globalThis.formatDate = index_js_1.formatDate;
        // @ts-ignore: setting globals
        globalThis.formatNumber = index_js_1.formatNumber;
        // @ts-ignore: setting globals
        globalThis.round = index_js_1.round;
        for (const key of plotMarks) {
            // @ts-ignore: setting globals
            dntShim.dntGlobalThis[key] = Plot[key];
        }
        for (const key of d3Functions) {
            // @ts-ignore: setting globals
            dntShim.dntGlobalThis[key] = d3[key];
        }
        // @ts-ignore: setup canvas
        document.createElement = ((orig) => (tagName) => {
            if (tagName.toLowerCase() === "canvas") {
                return (0, canvas_1.createCanvas)(1, 1);
            }
        ...
        if (element.tagName === "FIGURE" || element.nodeName === "FIGURE") {
            ...
            for (const child of children) {
                if (child.tagName === "SVG") {
                    ...
                    svgStrings.push(`<svg width="${chartWidth}" height="${height}" viewBox="0 0 ${chartWidth} ${height}"><image x="20" href="${dataUrl}" width="${width}" height="${height}" /></svg>`);
                }
                else if (child.classList.contains("plot-legend") ||
                    child.className?.includes("-legend") ||
                    child.className?.includes("-swatches")) {
                    // Handle HTML Legends (Categorical, etc.)
                    const items = child.querySelectorAll('[class*="-swatch"], [class*="-item"]');
                    if (items.length > 0) {
                        let legendSvg = `<svg width="${chartWidth}" height="${Math.ceil(items.length / 5) * 20 + 10}" viewBox="0 0 ${chartWidth} ${Math.ceil(items.length / 5) * 20 + 10}">`;
                        ...
                        items.forEach((item) => {
                            ...
                            if (swatch) {
                                legendSvg += swatch.outerHTML.replace("<svg ", `<svg x="${currentX}" y="${currentY}" `);
                            }
                            legendSvg += `<text x="${currentX + 20}" y="${currentY + 12}" font-size="12" fill="${options.dark ? "#B0B0B0" : "currentColor"}">${text}</text>`;
                            currentX += itemWidth;
                        });
                        legendSvg += `</svg>`;
        ...
        for (const svgHtml of svgStrings) {
            ...
            if (heightMatch)
                svgHeights.push(parseFloat(heightMatch[1]));
            else
                svgHeights.push(400);
        }
      ...
      <style>
        ...
        let masterSvg = `<svg width="${chartWidth}" height="${totalHeight}" viewBox="0 0 ${chartWidth} ${totalHeight}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ${styleTag}
      ${background}
    `;
        for (const comp of components) {
            if (comp.type === "text") {
                const x = 20;
                masterSvg +=
                    `<text x="${x}" y="${comp.y}" font-size="${comp.fontSize}" fill="${comp.fill}" text-anchor="${comp.anchor}" ${comp.className ? `class="${comp.className}"` : ""}>${comp.html}</text>`;
            }
            else {
                const xOffset = 20;
                masterSvg += comp.html.replace(/<svg/i, `<svg x="${xOffset}" y="${comp.y}" overflow="visible"`);
            }
        }
        masterSvg += `</svg>`;
        ...
        if (extension === "svg") {
            (0, node_fs_1.writeFileSync)(path, masterSvg);
        }
        else if (extension === "png") {
            const resvg = new resvg_js_1.Resvg(masterSvg, {
            ...
            (0, node_fs_1.writeFileSync)(path, pngBuffer);
        }
        else {
