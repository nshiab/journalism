...
exports.default = askAIPool;
const askAI_js_1 = __importDefault(require("./askAI.js"));
 ...
 * @category AI
 */
async function askAIPool(requests, poolSize, poolOptions = {}) {
    ...
    const worker = async () => {
        while (queue.length > 0) {
            ...
            try {
                const result = await (0, askAI_js_1.default)(req.prompt, {
                    ...req.options,
                ...
                if (poolOptions.logProgress) {
                    ...
                    const metricsInfo = poolOptions.metrics
                        ? ` | Cost so far: $${(0, index_js_1.formatNumber)(poolOptions.metrics.totalCost, {
                            significantDigits: 6,
                        })}`
                        : "";
            ...
            if (poolOptions.minRequestDurationMs) {
                await (0, sleep_js_1.default)(poolOptions.minRequestDurationMs, requestStart);
            }
        }
    };
