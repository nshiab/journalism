 ...
 * @category Finance
 */
export default function mortgageMaxAmount(annualIncome, downPayment, rate, options = {}) {
...
function findMaxAmount(startPrice, increment, monthlyIncome, downPayment, monthlyRate, amortizationPeriodinMonths, monthlyHeating, monthlyCondoFees, monthlyDebtPayment, options, results) {
    ...
    for (let purchasePrice = startPrice; purchasePrice <= 10_000_000; purchasePrice += increment) {
        ...
        const insurancePremium = mortgageInsurancePremium(purchasePrice, downPayment);
