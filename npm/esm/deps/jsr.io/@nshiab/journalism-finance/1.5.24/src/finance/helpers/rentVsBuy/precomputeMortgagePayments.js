...
export default function precomputeMortgagePayments(numberOfYears, startingMortgageAmount, fixedRateAdjustment, variableRateAdjustment, fixedInterestRates, variableInterestRates, floorRate) {
    const TERM_YEARS = 5;
    ...
    for (let month = 0; month < numberOfYears * 12; month += TERM_MONTHS) {
        const effectiveInterestRate = round(Math.max(floorRate, fixedInterestRates[month] + fixedRateAdjustment) *
            100, { decimals: 2 });
        ...
        const payments = mortgagePayments(mortgageAmount, effectiveInterestRate, "monthly", TERM_YEARS, AMORTIZATION_YEARS - (month / 12), { decimals: 2 });
    ...
    for (let month = 0; month < numberOfYears * 12; month += TERM_MONTHS) {
        ...
        for (let i = 0; i < TERM_MONTHS; i++) {
            monthlyEffectiveRates[i] = round(Math.max(floorRate, variableInterestRates[month + i] + variableRateAdjustment) * 100, { decimals: 2 });
        }
        const payments = variableMortgagePayments(mortgageAmount, monthlyEffectiveRates, TERM_YEARS, AMORTIZATION_YEARS - (month / 12), {
            decimals: 2,
        });
        for (let i = 0; i < payments.length; i++) {
            const payment = payments[i];
