...
import { GenerateContentResponse, GoogleGenAI, HarmBlockThreshold, HarmCategory, ThinkingLevel, } from "@google/genai";
import { formatNumber, prettyDuration } from "../../../../journalism-format/1.1.7/src/index.js";
import crypto from "node:crypto";
import ollama, { Ollama } from "ollama";
// Implementation
export default async function askAI(prompt, options = {}) {
    ...
    if (options.image) {
        const imageFiles = Array.isArray(options.image)
            ...
            : [options.image];
        if (ollamaVar) {
            message.images = imageFiles.map((imageFile) => readFileSync(imageFile, {
                encoding: "base64",
            }));
        }
        else {
    ...
    const params = {
        ...
        config: {
            ...
            thinkingConfig: options.thinkingLevel
                ? {
                    thinkingLevel: ThinkingLevel[options.thinkingLevel.toUpperCase()],
    ...
    if ((options.verbose || options.metrics || options.detailedResponse) &&
        ...
        if (!modelPricing) {
            ...
            if (options.verbose) {
                console.log(`\n\nTokens in:`, formatNumber(detailedResponse.promptTokenCount), "/", "Tokens out:", formatNumber(detailedResponse.outputTokenCount), "/", "Thinking tokens:", formatNumber(detailedResponse.thoughtsTokenCount), "/", "Tokens per second:", formatNumber(detailedResponse.tokensPerSecond, {
                    significantDigits: 1,
                }), "/", `Estimated cost${options.webSearch ? " (web search excluded)" : ""}:`, formatNumber(detailedResponse.estimatedCost, {
                    prefix: "$",
                    significantDigits: 1,
                    suffix: " USD",
                }));
        ...
        if (options.verbose) {
            console.log(`\n\nTokens in:`, formatNumber(detailedResponse.promptTokenCount), "/", "Tokens out:", formatNumber(detailedResponse.outputTokenCount), "/", "Thinking tokens:", "N/A", "/", "Tokens per second:", formatNumber(detailedResponse.tokensPerSecond, {
                significantDigits: 1,
            }));
        }
    }
