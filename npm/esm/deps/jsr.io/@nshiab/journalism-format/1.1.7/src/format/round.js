 ...
 * @category Formatting
 */
export default function round(number, options = {}) {
    ...
    if (optionsToCheck.filter((d) => typeof d === "number").length > 1) {
        throw new Error("You can't use options decimals, nearestInteger, or significantDigits together. Pick one.");
    }
    if (typeof number !== "number") {
        if (options.try === true) {
            ...
            throw new Error(`${number} is not a number. If you want to return NaN instead of throwing an error, pass option {try: true}.`);
        }
    }
    if (typeof options.decimals === "number") {
        ...
        return (Math.round(number / options.nearestInteger) * options.nearestInteger);
    }
