...
import { getHtmlTable } from "../../src/index.js"

describe("getHtmlTable", () => {
    it("should return an array of objects from an HTML table with a selector option", async function () {
        const data = await getHtmlTable(
            "https://www.ourcommons.ca/proactivedisclosure/en/members/2024/1",
            { selector: "#data-table" }
        )

        assert.deepStrictEqual(data.slice(0, 5), [
            {
                Name: "Aboultaif, Ziad",
    ...
    it("should return an array of objects from an HTML table with an index option", async function () {
        const data = await getHtmlTable(
